...
class Modified_SSDLiteMobileViT(nn.Module):
        ...
        **kwargs,
        ...
        if pretrained is None:
            ...
            pretrained = os.path.join(
                ROOT_DIR, "models/coco-ssd-mobilevitv2-0.75_81nc_pretrained.pt"
            )
        ...
        self.aux_encoder: BaseImageEncoder | nn.Identity = (
            copy.deepcopy(self.model.encoder) if multimodal else nn.Identity()
        )
        ...
        self.prepostprocessing = prepostprocessing
        ...
        self.register_buffer(
            "phenotype_stds", torch.Tensor(phenotype_stds).unsqueeze(0)
        )
        self.register_buffer(
            "phenotype_means", torch.Tensor(phenotype_means).unsqueeze(0)
        )

        if boxcox_lambdas is not None:
            self.register_buffer(
                "boxcox_lambdas", torch.Tensor(boxcox_lambdas).unsqueeze(0)
            )
    ...
    def compute_loss(
        ...
        for (
            ...
            targets, bbox_regression, cls_logits, phenotypes_pred, anchors, matched_idxs
        ):
            ...
            if (
                "phenotypes" in targets_per_image
                and foreground_idxs_per_image.numel() > 0
                and model_outputs_phenotypes
            ):
                matched_phenotypes = targets_per_image["phenotypes"][
                    foreground_matched_idxs_per_image
                ]
    ...
    def forward(
        self,
        images: List[DualTensor | Tensor] | Tensor,
        ...
        if isinstance(images, List):
            return self.forward_with_prepostprocessing(images, targets)
        else:
            raise ValueError(
                "When prepostprocessing is enabled, images should be a list of Tensors or DualTensors."
            )
    ...
    def postprocess_detections(
        ...
        for boxes, scores, phenotypes, anchors, image_shape in zip(
            bbox_regression, pred_scores, phenotypes_pred, image_anchors, image_shapes
        ):
            ...
            for label in range(1, num_classes):
                ...
                if hasattr(self, "boxcox_lambdas"):
                    phenotype = torch.pow(
                        phenotype * self.boxcox_lambdas + 1, 1 / self.boxcox_lambdas
                    )
                if hasattr(self, "minimums") and hasattr(self, "maximums"):
                    phenotype = (
                        phenotype * (self.maximums - self.minimums)
                    ) + self.minimums  # min max scaling
    ...
    def forward_with_prepostprocessing(
        self,
        images: List[DualTensor | Tensor],
        targets: Optional[List[Dict[str, Tensor]]] = None,
    ) -> (
        Tuple[Dict[str, Tensor], List[Dict[str, Tensor]]]
        | Dict[str, Tensor]
        ...
        image_tensors: List[Tensor] = [
            item.x if isinstance(item, DualTensor) else item for item in images
        ]
        ...
        if self.multimodal:
            aux_tensors: List[Tensor] = [
                item.y if isinstance(item, DualTensor) else item for item in images
            ]
            aux_images_transformed, _ = self.transform(aux_tensors)
            features = self.get_backbone_features(
                images_transformed.tensors, aux_images_transformed.tensors
            )
        else:
        ...
        if self.training:
            ...
            detections = self.postprocess_detections(
                head_outputs, anchors, images_transformed.image_sizes
            )
            detections = self.transform.postprocess(
                detections, images_transformed.image_sizes, original_image_sizes
            )
...
def _resplit_to_c3c4c5_features(
    features: nn.Sequential, c3_pos=7, c4_pos=13
) -> nn.Sequential:
    all_layers = []
    ...
    return nn.Sequential(
        ...
        nn.Sequential(
            all_layers[c3_pos].block[1:],
            *all_layers[c3_pos + 1 : c4_pos],
            all_layers[c4_pos],
        ),
...
def ssdlite320_dual_mobilenet_v3_large(
    ...
    **kwargs: Any,
    ...
    model.head.phenotype_head = SSDLitePhenotypeHead(
        out_channels, num_anchors, norm_layer
    )
    ...
    def modified_forward(
        ...
        if self.training:
            ...
            detections = self.transform.postprocess(
                detections, images_transform.image_sizes, original_image_sizes
            )
            # returns a list of detections
...
class PhenotypeRegressor(nn.Module):
    def __init__(
        ...
        self.transform = transforms.Compose(
            [transforms.Resize(size), transforms.Normalize(image_mean, image_std)]
        )
    ...
    def forward(self, input: DualTensor) -> Tensor:
        ...
        if self.dual_branch:
            ...
            features = torch.cat(
                (features, y_features), dim=1
            )  # Concatenate [B, 576] and [B, 576]
